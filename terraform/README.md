...
| `game_port` | `8080` |
| `quorum_votes` / timings | see `docs/game-loop.md` (prod quorum 2, countdown 15, betting close after video start 5, video timeout 300, settle 8) |
...
| `WORLD_ID_ENVIRONMENT` | `TF_VAR_world_id_environment` | from `.env`: `production`, `staging`, or `sandbox` (operator: `sandbox`) |
| `WORLD_ID_STAGING_TOKEN` | `TF_VAR_world_id_staging_token` | from `.env` when environment is `staging`; empty string otherwise (not set on the app) |
