window.HT = (() => {
  ...
  function paint(canvas, w, h, layers) {
    canvas.width = w;
    canvas.height = h;
    off.width = w;
    off.height = h;
    ...
    for (let i = 0; i < w * h; i++) {
      if (lit[i]) continue;
      const x = i % w;
      const n = [x > 0 && i - 1, x < w - 1 && i + 1, i - w, i + w].find(
        (j) => j !== false && lit[j],
      );
      if (n !== undefined) o.set([o[n * 4], o[n * 4 + 1], o[n * 4 + 2], 70], i * 4);
    }
  ...
  const shoulders = (c) => A(c, 16, 37, 13, 1.12, 1.88);
  const eyes = (c) => D(c, 14, 13, 18, 13);
  const cap = (c) => A(c, 16, 11, 6, 1, 2);
  ...
  function portrait(canvas, kind, color, opts = {}) {
    const layers = [];
  ...
  return { paint, portrait, PORTRAITS, css, E, A, L, D };
})();
